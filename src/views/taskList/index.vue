...
import { taskListFundata } from '/@/api/taskList/index';

// 引入组件
const Table = defineAsyncComponent(() => import('/@/components/table/index.vue'));
const TableSearch = defineAsyncComponent(() => import('/@/components/table/search.vue'));
const EditDialog = defineAsyncComponent(() => import('/@/views/taskList/form/edit.vue'));
...
const state = reactive<TableDemoState>({
	tableData: {
		...
		header: [
			...
			{ key: 'logUrl', colWidth: '', title: 'TASK IMAGE',width: '40', height: '40', type: 'image', isCheck: true},
			{ key: 'points', colWidth: '', title: 'TASK INTEGRAL', type: 'text', isCheck: true },
			{ key: 'type', colWidth: '', title: 'TASK TYPE', type: 'text', isCheck: true },
			{ key: 'logUrl', colWidth: '', width: '40', height: '40', title: 'HEAD IMAGE', type: 'image', isCheck: true },
			{key:'url',colWidth:'',title:'path',isCheck:true}
		],
		// 配置项（必传）
		config: {
			total: 0, // 列表总数
		...
		// search: [
		// 	{ label: '任务名称', prop: 'realname', placeholder: '请输入姓名', required: false, type: 'input' },
		// ],
...
const getTableData = () => {
	...
	taskListFundata(state.tableData.param).then((response: any) => {
