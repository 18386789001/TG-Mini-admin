...
import { auditGetFun, auditPassTwo } from "/@/api/auditManagement/index"
import { get } from 'node_modules/_@types_sortablejs@1.15.8@@types/sortablejs';
...
const state = reactive<TableDemoState>({
	tableData: {
		// 列表数据（必传）
		data: [{id:1,name:'游戏任务',path:'123',component:'1',static:0}],
		// 表头内容（必传，注意格式）
		header: [
			{ key: 'id', colWidth: '', title: 'ID', type: 'text', isCheck: true },
            { key: 'createTime', colWidth: '', title: 'AUDIT TIEM', type: 'text', isCheck: true },
...
const getTableData = () => {
	...
	auditGetFun(state.tableData.param).then((response: any) => {
