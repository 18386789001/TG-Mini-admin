...
import { userWallet } from '/@/api/userWallet/index';
...
const state = reactive<TableDemoState>({
	tableData: {
		...
		header: [
			...
			{ key: 'createTime', colWidth: '', title: 'TIME', type: 'text', isCheck: true },
			{ key: 'userId', colWidth: '', title: 'USER ID', type: 'text', isCheck: true },
            { key: 'walletCategoryId', colWidth: '', title: 'WALLET CATEGORYID', type: 'text', isCheck: true },
            { key: 'walletUrl', colWidth: '', title: 'WALLET URL', type: 'text', isCheck: true }
		],
		// 配置项（必传）
		config: {
			total: 0, // 列表总数
...
const getTableData = () => {
	...
	userWallet(state.tableData.param).then((response: any) => {
